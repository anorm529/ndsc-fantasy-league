...
export default async function PlayersPage({
  ...
  searchParams: Promise<{ search?: string; gender?: string; team?: string }>;
}) {
  const { leagueId } = await params;
  const { search, gender, team } = await searchParams;
  ...
  return (
    <div className="space-y-6">
      ...
      <form method="GET" className="flex flex-wrap gap-3 items-center">
        ...
        {(search || gender || team) && (
          <a href={`/league/${leagueId}/players`} className="text-sm text-slate-500 hover:text-slate-700">
            Clear
          </a>
        )}
      </form>
      ...
      {fantasyTeam && (
        <div className="bg-ndsc-navy/5 border border-ndsc-navy/20 rounded-lg px-4 py-3 text-sm flex gap-6">
          <span>
            <strong className="text-ndsc-navy">Budget:</strong> £{budget.toFixed(1)}M remaining
          </span>
          <span>
            <strong className="text-ndsc-navy">Squad:</strong> {fantasyTeam.roster.length}/5 players
          </span>
          {squadFull && (
            <span className="text-amber-600 font-medium">Squad full — remove a player first</span>
          )}
        </div>
      )}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {players.map((p) => {
                ...
                const canAfford = meta.currentPrice <= budget;
